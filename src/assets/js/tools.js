   let methods = {};
    ...
    methods.imageZoom = function (dom, url, width, height) {
        ...
        // 获取元素在页面中的位置
        let positonByHtml = methods.offset(dom)
        // 函数体局部变量
    ...
    */
    ...
    methods.formatTime = function (format, date) {
        ...
        let result = {};
        let reg = /[ymd]+/g;
        let middle = format.split(reg);
        let y = format.indexOf('y');
        let m = format.indexOf('m');
        let d = format.indexOf('d');
        console.log(y, m, d)
        let returnResult = ''
        switch (true) {
            case y !== -1:
    ...
    methods.htmlTocanvas = (dom, options) => {
        ...
        function get_element_styles(element) {
